...
- [ ] Secrets from a vault, retrieved with a managed identity (spec `Requirements`) — **not what's actually live.** The schedule that's actually running (GitHub Actions) pulls secrets from GitHub Actions repo secrets, not Key Vault. Key Vault + managed identity is implemented in the `deploy/container-apps-job.bicep` / `deploy/README.md` path, but that path isn't deployed — it's an alternative deployment, not the one this project runs on today.
